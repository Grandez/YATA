modTestInput <- function(id) {
    ns <- NS(id)
    useShinyjs()
#    panelLeft = column(id = ns("left"), width=2, style="display: none", br()
    panelLeft = div(id = ns("left"), class="shinyjs-hide"
...
#                        hidden(textOutput(ns("msgError")))
                 ,menuTab(id, list("Last", "Ask", "Bid", "High", "Low", "Volume"))
,actionButton(ns("btn"), "Boton")
#      panelMain  = tagList(menuTab(id, list("Last", "Ask", "Bid", "High", "Low", "Volume")), hr(width="100%")
    
                        # ,boxPlus(id=ns("boxHeader"), title=textOutput(ns("lblHeader")), width=12, closable = FALSE,status = "primary"
                        #          ,solidHeader = TRUE
                        #          ,collapsible = TRUE,
                        #          enable_label = TRUE,
    ...
    #tagList(panelMain)
    list(left=panelLeft, main=panelMain)
}    
